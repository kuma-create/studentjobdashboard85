...
export default function JobEditForm({ job, company, userId }: JobEditFormProps) {
  ...
  const [location, setLocation] = useState(job.location || company.location || "")
  const [jobType, setJobType] = useState(job.job_type || "full_time")
  const [salaryMin, setSalaryMin] = useState(job.salary_range?.min?.toString() || "")
  const [salaryMax, setSalaryMax] = useState(job.salary_range?.max?.toString() || "")
  const [applicationDeadline, setApplicationDeadline] = useState(job.application_deadline || "")
  const [isRemote, setIsRemote] = useState(job.location === "リモート")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "エラーが発生しました",
        description: error.message || "求人の更新中にエラーが発生しました",
        variant: "destructive",
      })
    } finally {
